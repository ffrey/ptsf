...
# Protected
  ...
  {
  	/*
  	  	// ffrey 2011 02 11 : Compilation failed: missing ) at offset 23
//  	  	print 'DATA BRUT : ' . $root_path . ' / ' . $requested_path . '<br/>';
//  	  	print 'data realpath : ' . realpath($root_path) . ' / ' . realpath($requested_path);
  	// var_dump(realpath($root_path), realpath($requested_path)); exit;
//  	
$root = str_replace('\\', '_', realpath($root_path) );
//  	
$req  = str_replace('\\', '_', realpath($requested_path) );


var_dump($req, $root);
return preg_match('`^'.realpath($root).'`', realpath($req));
    return preg_match('`^'.realpath($root_path).'`', realpath($requested_path));
//    return preg_match('`^'.realpath($root_path).'`', realpath($requested_path));
*/
    ## @author Guéric
    $root_path2 = str_replace('\\','\\\\',realpath($root_path));
    return preg_match('`^'.$root_path2.'`', realpath($requested_path));
  }
